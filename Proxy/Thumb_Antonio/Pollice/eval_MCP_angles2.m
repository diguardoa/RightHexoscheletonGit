...
eq5 = K_MCP -( K_PIP + l_proxi^2 - 2*sqrt(K_MCP)*l_proxi*cos(PIP_FE));
...
eq8 = PIP_CMC - (l_meta^2 + l_proxi^2 - 2*l_meta*l_proxi*cos(MCP_FE));% == 0;
load eqPIP.mat
% Eq1
%eq8 = eqPIP(1);
%eq9 = eqPIP(2);
...
J = jacobian(F,[xK,yK,zK,xPIP,yPIP,zPIP,PIP_FE,MCP_FE, MCP_AA])
J_inv = inv(J)
