...
eq5 = l_meta^2*(K_MCP - K_PIP - l_dist^2)^2 -( K_PIP*(PIP_CMC - l_meta^2 - l_dist^2)^2);% == 0;
...
%eq9 = eqPIP(2);
% %% Risolvo sistema
[xK_s,yK_s,zK_s,xPIP_s,yPIP_s,zPIP_s] = solve([eq1;eq2;eq3;eq4;eq5;eq6]==zeros(6,1),...
...
% %%
% eq10 = K_MCP -( K_PIP + l_dist^2 - 2*sqrt(K_MCP)*l_dist*cos(MCP_FE)) == 0;
% [MCP_FE_s] = solve(eq10,MCP_FE);
% MCP_FE_s = MCP_FE_s(1)
% matlabFunction(MCP_FE_s,'file','findMCP_FE')
% %     [xK,yK,zK,xPIP,yPIP,zPIP]);
