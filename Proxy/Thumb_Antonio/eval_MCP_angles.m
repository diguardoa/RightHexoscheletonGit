...
% Output
syms xPIP yPIP zPIP xK yK zK PIP_FE MCP_FE

% Before:
% scrivo distanza K_PIP
K_PIP = sqrt( (xPIP - xK)^2 + (yPIP - yK)^2 + (zPIP - zK)^2 );
% scrivo distanza PIP_MCP
PIP_MCP = sqrt( (xPIP - xMCP)^2 + (yPIP - yMCP)^2 + (zPIP - zMCP)^2 );
% scrivo distanza K_MCP
K_MCP = sqrt( (xK - xMCP)^2 + (yK - yMCP)^2 + (zK - zMCP)^2 );
% scrivo distanza K_G2
K_G2 = sqrt( (xK - xG2)^2 + (yK - yG2)^2 + (zK - zG2)^2 );
% scrivo distanza PIP_CMC
PIP_CMC = sqrt( (xPIP - xCMC)^2 + (yPIP - yCMC)^2 + (zPIP - zCMC)^2 );
...
eq3 = K_G2^2 + K_PIP^2 == (xPIP - xG2)^2 + (yPIP - yG2)^2 + (zPIP - zG2)^2;
...
eq5 = K_MCP^2 == K_PIP^2 + l_proxi^2 - 2*K_MCP*l_proxi*cos(PIP_FE);
...
eq7 = PIP_CMC^2 == l_meta^2 + l_proxi^2 - 2*l_meta*l_proxi*cos(MCP_FE);

% 8. relazione PIP_FE e MCP_FE
eq8 = PIP_FE == MCP_FE;

%% Risolvo sistema
[xK_s,yK_s,zK_s,xPIP_s,yPIP_s,zPIP_s,MCP_FE_s,PIP_FE_s] = solve([eq1;eq2;eq3;eq4;eq5;eq6;eq7;eq8],...
    [xK,yK,zK,xPIP,yPIP,zPIP,MCP_FE,PIP_FE]);

%% Ottimizzo la soluzione in un file
matlabFunction(xK_s,yK_s,zK_s,xPIP_s,yPIP_s,zPIP_s,MCP_FE_s,PIP_FE_s,'file','findAngles_optimized')
